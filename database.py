...
class dbworker:
	...
	def create_profile(self,telegram_id,telegram_username,name,description,city,photo,sex,age,social_link, tags):
		'''Создаём анкету'''
		with self.connection:
			return self.cursor.execute("INSERT INTO `profile_list` (`telegram_id`,`telegram_username`,`name`,`description`,`city`,`photo`,`sex`,`age`,`social_link`, `tags`) VALUES(?,?,?,?,?,?,?,?,?,?)", (telegram_id,telegram_username,name,description,city,photo,sex,age,social_link,tags))
	...
	def edit_tags(self,tags,user_id):
		'''изменение описания'''
		with self.connection:
			return self.cursor.execute('UPDATE `profile_list` SET `tags` = ? WHERE `telegram_id` = ?',(tags,user_id))
	...
	def edit_photo(self,photo,user_id):
		'''изменение возвраста'''
		with self.connection:
			return self.cursor.execute('UPDATE `profile_list` SET `photo` = ? WHERE `telegram_id` = ?',(photo,user_id))
	def search_profile(self,city):
		...
		try:
			...
			with self.connection:
				...
				return self.cursor.execute("SELECT `telegram_id` FROM `profile_list` WHERE `city` = ?",(city,)).fetchall()
	...
	def set_city_search_slava(self,city):
		'''задования города для поиска'''
		with self.connection:
			return self.cursor.execute('SELECT * FROM profile_list WHERE city = ?',(city,)).fetchall()
	...
	def delete_report(self,sender,recipent):
		'''добавление лайка в таблицу'''
		with self.connection:
			return self.cursor.execute('DELETE FROM `reports` WHERE `send` = ? AND `recipient` = ?', (sender,recipent))
	...
	def city_search_exists(self,user_id):
		...
		with self.connection:
			...
			return result
	def delete_like(self,sender,recipent):
		'''добавление лайка в таблицу'''
		with self.connection:
			return self.cursor.execute('DELETE FROM `likes` WHERE `sender` = ? AND `recipient` = ?', (sender,recipent))
	...
	def add_like_exists(self,sender,recipient):
		with self.connection:
			...
			return bool(len(result))
	def delete_dislike(self,sender,recipent):
		'''добавление лайка в таблицу'''
		with self.connection:
			return self.cursor.execute('DELETE FROM `dislikes` WHERE `sender` = ? AND `recipient` = ?', (sender,recipent))
	def add_dislike(self,sender,recipent):
		'''добавление лайка в таблицу'''
		with self.connection:
			return self.cursor.execute('INSERT INTO `dislikes` (`sender`,`recipient`) VALUES(?,?)', (sender,recipent))
	def add_dislike_exists(self,sender,recipient):
		with self.connection:
			result = self.cursor.execute('SELECT * FROM `dislikes` WHERE `sender` = ? AND `recipient` = ?', (sender,recipient)).fetchall()
			return bool(len(result))
